...
export default {
  ...
  data() {
    ...
    return {
      ...
      searchForm: [
        { type: "input", placeholder: "管理员名称", prop: "username", width: 200 },
      ...
      tableHandles: [
        {
		  ...
		  isShow: e => this.checkBtn('sys:user:save,sys:role:select')
        }
      ],
      tableData: [],
      tableCols: [
        ...
        { label: "创建时间", prop: 'createTime', formatter: e => { return this.parseTime(e.createTime) } },
		{
			...
			isDisabled: e => !this.checkBtn('sys:user:update,sys:role:select')
		},
        {
          label: "操作",
          type: "button",
          ...
          btnList: [
            {
			  ...
			  isShow: e => this.checkBtn('sys:user:update,sys:role:select')
            },
            {
              label: "删除",
              type: "danger",
			  ...
			  isShow: e => this.checkBtn('sys:user:delete')
            }
          ]
        },
      ],
      ...
      formCols: [
        { type: "input", label: "管理员名称", width: 350, prop: "username" },
		...
		{ type: "select", label: "管理门店", width: 350, prop: "storeIds", multiple: true },	
  ...
  methods: {
	...
	async getStoreList() {
      ...
      this.formCols[this.labIndex(this.formCols, '管理门店')].options = list;
	},
    async getData() {
