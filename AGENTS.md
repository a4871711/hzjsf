...
健身房管理系统 **hzjsf**,前后端一体的 monorepo,两个独立子项目各自有更详细的 AGENTS.md——**动到哪个子项目就先读它的 AGENTS.md**,本文件只讲跨项目的大局,不重复其中细节。
...
- **移动端 App / 小程序** → 走后端 **`api` 模块**(`/api/**`)。鉴权**不在拦截器**,而在 `BaseController` 里按 token 查 Redis(详见 `Backend/AGENTS.md`「API 鉴权」)。⚠️ 本仓库**不含**移动端工程,只有后端接口和 `Backend/h5/` 里的若干 H5 页面。
