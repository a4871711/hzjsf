...
public class ShiroConfig {
	...
	@Bean("authc") // 覆盖默认的 authc 过滤器
    public UnifiedAuthFilter unifiedAuthFilter() {
        return new UnifiedAuthFilter();
    }
	...
	public SessionManager sessionManager(SimpleCookie sessionIdCookie,RedisShiroSessionDAO redisShiroSessionDAO, @Value("${renren.redis.open}") boolean redisOpen, @Value("${renren.shiro.redis}") boolean shiroRedis) {
		...
		// 设置session过期时间为1小时(单位：毫秒)，默认为30分钟  (1个月)
	...
	public ShiroFilterFactoryBean shirFilter(SecurityManager securityManager) {
        ...
        filterMap.put("/sys/prologin", "anon");
        filterMap.put("/sys/forgetPassword", "anon");
