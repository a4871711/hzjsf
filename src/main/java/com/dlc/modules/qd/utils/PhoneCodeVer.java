...
public class PhoneCodeVer {
    ...
    public static String getCode(String phoneNum){
        ...
        redisUtils.set(ConfigConstant.PHONE+phoneNum,code,600);
        return code;
   ...
   // ------------------身份证格式的正则校验----------------->
    ...
    public static String sendCode(String phoneNum){
        ...
        if(isPhoneNum(phoneNum)){
            ...
            try {
                ...
                String tkey = getTkey();
                String content = "你好，你的验证码是:"+getCode(phoneNum)+"【S-live运动】";
                content = URLEncoder.encode(content, "UTF-8");
                Map<String, Object> map = new HashMap<>();
                map.put("username", ConfigConstant.ZT_USERNAME);
                map.put("password", getPassword(ConfigConstant.ZT_PASSWORD,tkey));
                map.put("mobile", phoneNum);
                map.put("content", content);
                map.put("tkey", tkey);
                map.put("xh", "");
                result = SendPushPost.sendPost(ConfigConstant.ZT_PHONE_VER_URL, map);
    ...
    public static String getUrl(String phoneNum) throws UnsupportedEncodingException {
        ...
        String tkey = getTkey();
        String url = ConfigConstant.ZT_PHONE_VER_URL+"?username="+username+"&tkey="+tkey
