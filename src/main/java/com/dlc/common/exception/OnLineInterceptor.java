...
public class OnLineInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object o) throws RRException {
		...
		UserInfoVo vo = (UserInfoVo) request.getSession().getAttribute(ConfigConstant.ACCOUNT);
		//logger.info("OnLineInterceptor---userVo->"+vo);
		if(vo != null){
		    String sessionID = redisUtils.get(ConfigConstant.USER+vo.getUserId());
		    if(sessionID == null){throw new RRException(CodeAndMsg.ERROR_USER_NOT_LOGIN);}
