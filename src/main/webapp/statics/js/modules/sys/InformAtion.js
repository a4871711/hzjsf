$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [
            ...
            {label: '资讯类型', name: 'infType', index: 'infType', width: 80,
            ...
            {
                label: '发布时间', name: 'publishTime', index: 'publishTime', width: 80,
                formatter: function (value) {
          ...
          /*  {label: '内容', name: 'content', index: 'content', width: 50},*/
            {
    ...
    $.each($(".upload"), function () {
        var _this = $(this);
        new AjaxUpload($(this), {
            action: baseURL + "sys/upload?width=220&height=200",
            name: 'files',
            autoSubmit: true,
            responseType: "json",
...
var vm = new Vue({
    ...
    methods: {
        ...
        saveOrUpdate: function (event) {
            vm.list.content = ue.getContent();
            vm.list.infImgUrl = $("#img img").attr("src");
            if (vm.validator()) {
                return;
            }
        ...
        getInfo: function (informationId) {
            $.get(baseURL + "sys/Inform/info/" + informationId, function (r) {
                ...
                ue.setContent(r.sysInformAtionEntity.content);
        ...
        validator: function () {
            vm.list.infImgUrl = $("#img img").attr("src");
            if (vm.list.publishTime==null || vm.list.publishTime==""){
                alert("请选择发布时间");
                return true;
            }
            if (isBlank(vm.list.title)){
                alert("请输入标题");
                return true;
            }
            if (vm.list.title.length>30){
                alert("标题名称请输入20个字符串内");
                return true;
            }
            console.log(isNaN(vm.list.publishTime))
            if(vm.list.publishTime==null || vm.list.publishTime==""||isNaN(vm.list.publishTime)){
                alert("请选择发布时间");
                return true;
            }
