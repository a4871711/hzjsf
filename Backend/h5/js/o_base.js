...
var baseUrl = "https://shilijsf.shilisports.com/";
        ...
        function (res) {
            if (res.errmsg == "ok") {
                ...
                var nonceStr = Math.random()
                    .toString(36)
                    .substr(2);
                var tickets = wx_js(ticket, timestamp, nonceStr);
                wx.config({
                    debug: false, // 开启调试模式,调用的所有api的返回值会在客户端//alert出来，若要查看传入的参数，可以在pc端打开，参数信息会通过log打出，仅在pc端时才会打印。
                    appId: "wxfc03306f16362704", // 必填，公众号的唯一标识
    ...
    if (!getItem('userInfo')) {
        if (getParam().code) {
            ...
            ajaxPost({
                ...
                "api/userInfo/h5Login",
                function (res) {
                    console.log('获取token', res)
                    if (res.code == 1) {
                        setItem('userInfo', JSON.stringify(res.data.userInfo))
                    ...
                    if (call) {
                        ...
                        goTo("o_cardList.html")
                    }
                }, false)
        } else {
            window.location.replace('https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxfc03306f16362704&redirect_uri=' + window.location.href + '&response_type=code&scope=snsapi_userinfo&state=1#wechat_redirect')
        }
    } else {
        if (call) {
            call()
...
function overHide(content, contentDom, allDom) {
    var showLittle = content.substring(0, 50);
    $("." + contentDom).text(showLittle + "...");
...
function buyNow() {
    $(".mask").show();
    $(".payMaskBox").slideToggle();
}

function openCoupon() {
    $(".payMaskBox").slideToggle();
    $(".couponMaskBox").slideToggle();
}

function shop() {
    $(".mask").show();
    $(".shopMaskBox").slideToggle();
}
...
function changeGetWay() {
    var getWay = $("input[name='getWay']:checked").val();
    switch (getWay) {
        case "0":
            $(".addressLi").addClass("hideImportant")
            delete param.userAddressId;
            param.sendType = 1
            break;
        case "1":
            $(".addressLi").removeClass("hideImportant")
            param.userAddressId = getParam().addressId;
            param.sendType = 0
            break;
    }
}