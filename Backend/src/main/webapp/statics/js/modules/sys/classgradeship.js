$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sys/classGrade/list',
        datatype: "json",
        colModel: [
            ...
            {label: '名称', name: 'dataName', index: 'dataName', width: 50},
            {label: '教练等级', name: 'price', index: 'price', width: 50},
            {label: '教练提成比例(%)', name: 'nextVal', index: 'nextVal', width: 50,
                formatter: function (value) {
                    return value == null? "--" : value+"%"
                }
            },
            {label: '课程最高售价', name: 'val', index: 'val', width: 50,
                formatter: function (value) {
                    return value == null? "--" : value+"元/节"
                }
            },
            {
...
var vm = new Vue({
    ...
    methods: {
        ...
        add: function () {
            $("#gradeFlag").attr("disabled",false);
            vm.showList = false;
            vm.title = "新增";
            vm.dataMapEntityList = {};
        },
        update: function (event) {
            $("#gradeFlag").attr("disabled",true);
        ...
        saveOrUpdate: function (event) {
            ...
            var url = vm.dataMapEntityList.dataMapId == null ? "sys/classGrade/save" : "sys/classGrade/update";
            $.ajax({
                type: "POST",
                url: baseURL + url,
        ...
        del: function (event) {
            ...
            confirm('确定要删除选中的记录？若已存在此等级下的教练、私教、私教订单，删除后将会受到影响。请谨慎操作', function () {
                $.ajax({
                    type: "POST",
                    url: baseURL + "sys/classGrade/delete",
                    contentType: "application/json",
        ...
        validator: function () {
            ...
            if (vm.dataMapEntityList.nextVal==null || !r.test(vm.dataMapEntityList.nextVal) ||
                    vm.dataMapEntityList.nextVal < 0 || vm.dataMapEntityList.nextVal>100) {
                layer.msg('请输入100内的正整数');
                return true;
            }
        }
    }
