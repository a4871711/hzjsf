$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sys/braceletset/list',
        datatype: "json",
        colModel: [
            {label: '序号', name: 'dataMapId', index: 'dataMapId', width: 50,key: true },
            {label: '手环名称', name: 'dataName', index: 'dataName', width: 80},
            {label: '手环价格', name: 'price', index: 'price', width: 50,
                formatter: function (value) {
                    return value == null? "--" : value+"元"
                }
            },
            { label: '描述', name: 'description', index: 'description', width: 50,
    ...
    methods: {
        ...
        del: function (event) {
            ...
            confirm('确定要删除选中的记录？', function () {
                $.ajax({
                    type: "POST",
                    url: baseURL + "sys/braceletset/delete",
                    contentType: "application/json",
        ...
        validator: function () {
            var retype = /^[1-9]\d*$/;
            if(isBlank(vm.dataMapEntityList.dataName)){
                layer.msg('请输入手环名称');
                return true;
            }
            if(vm.dataMapEntityList.dataName.length>20){
                layer.msg('名称过长');
                return true;
            }
            if(isBlank(vm.dataMapEntityList.price)){
                layer.msg('请输入价格');
                return true;
            }
            if (!retype.test(vm.dataMapEntityList.price)) {
                layer.msg("请输入整数价格");
                return true;
            }
