...
### ~~2.7 `store` 增加门店等级~~ 已废弃（适用门店用 `vip_benefit_card.store_addr_ids`）

转让适用门店校验并入转让前置校验，无需改 `store` 表、无需新增任何字段：权益的「适用门店」即权益卡商品 `vip_benefit_card.store_addr_ids`（逗号分隔的 `store_addr_id`，本就存在），转让时取受让人当前所属门店 `user_info.nowStoreId`（该字段存的就是 `store_addr_id`）判断是否在其中。
...
| `ERROR_VIP_STORE_NOT_APPLICABLE` | -58 | 受让人门店不在该权益适用的门店范围内 |
...
- 可能返回码：`ERROR_LACK_PARAM`(-1)、`ERROR_USER_IS_LOCK`(102)、`ERROR_VIP_BENEFIT_NOT_EXIST`(-51)、`ERROR_VIP_BENEFIT_NOT_OWNER`(-52)、`ERROR_VIP_BENEFIT_EXPIRED`(-53)、`ERROR_VIP_BENEFIT_NOT_NORMAL`(-54)、`ERROR_VIP_NOT_TRANSFERABLE`(-55)、`ERROR_VIP_TO_USER_NOT_EXIST`(-56)、`ERROR_VIP_TO_USER_SELF`(-57)、`ERROR_VIP_STORE_NOT_APPLICABLE`(-58)、`ERROR_VIP_USER_BLACKLIST`(-59)、`ERROR_VIP_FEE_RULE_NOT_EXIST`(-60)、`ERROR_PAY_ERROR`(-26)
- 逻辑说明：发起前置校验（权益属本人且 `status=0`/未过期、`transferable=1`、转让人/受让人均不在 `member_blacklist`、双方 `auditStatus!=2`、受让人当前门店 `nowStoreId`（即 `store_addr_id`）∈ 权益卡 `store_addr_ids`、不能转给自己）；后端按 `transfer_count+1` 重算 `service_fee`，建 `vip_benefit_transfer`。`service_fee>0`→`status=10`待付费，生成服务费单号（末位 `VIP_TRANSFER_FEE_TYPE`="7"）写入 `service_fee_order_no` 并返回微信下单参数；服务费支付成功回调（按后缀 "7" 分发）把 `status` 10→20待审核。`service_fee=0`→直接 `status=20`。审核通过流转见 5.8/5.9，确认生效是后台审核通过后由受让人 confirm 触发，过户单事务完成。
...
新增控制器：`SysVipCardController`、`SysVipFeeRuleController`、`SysVipTransferController`、`SysMemberBlacklistController`、`SysCardPauseController`。（适用门店校验由权益卡 `store_addr_ids` 决定，`SysStoreController` 无需改造。）
...
3. **通过分支（pass=1）**：
   - 复核前置校验规则集（权益未过期、双方未被封禁/拉黑、受让人门店在权益适用门店内等，与 api 发起侧同一套校验方法，避免审核期内状态变化放过非法单）；命中任一条则按驳回处理并写明原因。
...
### ~~6.4 门店等级 `SysStoreController` 改造~~ 已废弃

转让的「适用门店校验」并入转让前置校验（见 5.6 发起、6 审核章），由权益卡 `vip_benefit_card.store_addr_ids`（适用门店）决定：取受让人当前所属门店 `user_info.nowStoreId`（该字段实际存的就是 `store_addr_id`）判断是否在其中。门店表 `store` 无需新增任何字段，`SysStoreController` 无需改造，`admin` 的 `store.vue` 不动。
...
- `ERROR_VIP_STORE_NOT_APPLICABLE(-54, "受让人门店不在该权益适用的门店范围内")`
...
apply(fromUserId, benefitId, toUserId):
    ...
    校验 受让人门店在适用门店内 toUser.nowStoreId(=store_addr_id) ∈ card.store_addr_ids // ERROR_VIP_STORE_NOT_APPLICABLE
    ...
    t.fromStoreId = vb.storeId; t.toStoreId = toUser.nowStoreId   // 双方门店留痕(nowStoreId 即 store_addr_id)
...
| `ERROR_VIP_STORE_NOT_APPLICABLE` | -64 | 受让人门店不在该权益适用的门店范围内 |
 ...
 * 注：适用门店校验只需受让人当前门店(toUser.nowStoreId，即 store_addr_id)与权益卡 store_addr_ids，无需门店对象。
 */
void checkTransferable(VipBenefit benefit, UserInfo fromUser, UserInfo toUser);
```

**为什么 apply 和 audit 都要跑**：发起到审核之间有时间差（R-12 门店 1–2 工作日审核），其间权益可能过期、转让人/受让人可能被拉黑或封禁、权益卡可能下架、受让人当前所属门店可能变动。审核时必须用**审核当下**的数据重跑全部规则，命中即驳回（status→31 已驳回并触发退费，R-11）。apply 命中则直接拦截，转让单不创建、服务费支付单不生成。
...
| 16 | 适用门店：受让人当前门店 `toUser.getNowStoreId()`（基本类型 `int`，即 `store_addr_id`）不在该权益来源权益卡 `card.getStoreAddrIds()`（规则 15 已查出的 `vip_benefit_card`，逗号分隔的 `store_addr_id`，逐项 trim 后比对、对齐 `find_in_set`）内，或受让人未归属门店（`nowStoreId<=0`） | R-01 | `ERROR_VIP_STORE_NOT_APPLICABLE` |
...
public void checkTransferable(VipBenefit benefit, UserInfo fromUser, UserInfo toUser) {
    ...
    // 16. 适用门店:受让人当前所属门店 ∈ 该权益来源权益卡的适用门店 (R-01)
    //     card 为规则 15 已查出的 vip_benefit_card;nowStoreId 是基本类型 int(即 store_addr_id,未归属门店为 0)
    //     比对口径对齐系统既有多值门店匹配(SQL find_in_set,见 CouponMapper.xml):逐项 trim 后精确匹配;
    //     落地亦可直接用 mapper 的 find_in_set(#{toStoreAddrId}, store_addr_ids) 在 SQL 层判,避免空格/空项歧义
    int toStoreAddrId = toUser.getNowStoreId();
    String applicableAddrIds = card.getStoreAddrIds();
    boolean applicable = false;
    if (toStoreAddrId > 0 && applicableAddrIds != null && !applicableAddrIds.trim().isEmpty()) {
        for (String addrId : applicableAddrIds.split(",")) {
            if (addrId.trim().equals(String.valueOf(toStoreAddrId))) { applicable = true; break; }
        }
    }
    if (!applicable) {   // 含受让人未归属门店(nowStoreId<=0)→拦截
        throw new RRException(CodeAndMsg.ERROR_VIP_STORE_NOT_APPLICABLE.getMsg(),
                CodeAndMsg.ERROR_VIP_STORE_NOT_APPLICABLE.getCode());
    }
}
...
| 🔧 `com.dlc.common.utils.CodeAndMsg` | 新增码（现有负码到 -36，从 -50 起避开）。本功能实际用到：`ERROR_VIP_CARD_OFF_SHELF(-50,"权益卡已下架")`、`ERROR_VIP_BENEFIT_NOT_FOUND(-51,"权益不存在或不属于本人")`、`ERROR_VIP_BENEFIT_EXPIRED(-52,"权益已过期，无法转让")`、`ERROR_VIP_BENEFIT_NOT_TRANSFERABLE(-53,"该权益当前不可转让")`、`ERROR_VIP_TRANSFER_SELF(-54,"不能转让给自己")`、`ERROR_VIP_STORE_NOT_APPLICABLE(-55,"受让人门店不在该权益适用的门店范围内")`、`ERROR_VIP_USER_BLACKLIST(-56,"账号在黑名单中，不可转让")`、`ERROR_VIP_TRANSFER_STATUS(-57,"转让单状态不允许该操作")`、`ERROR_VIP_TRANSFER_NOT_OWNER(-58,"无权操作该转让单")`、`ERROR_VIP_PAUSE_MONTH_LIMIT(-59,"本月停卡次数已用完")`、`ERROR_VIP_PAUSE_YEAR_LIMIT(-60,"全年停卡次数已用完")` |
...
> 新增菜单后需给对应角色授权（`sys_role_menu`）。

---

## 11. 落地顺序与自测清单

落地原则：先做相互独立、可单独验收的小项（权益卡购买含人数与动态涨价 / 停卡 / 黑名单），把支付回调链路打通；最后做转让主流程与审核（依赖前面的权益实例与支付回调）。

### 11.1 落地顺序

1. **建表与脚本**：执行 `Backend/sql/` 两个脚本（6 张新表 + `sys_menu`）。
2. **公共改动**：`ConfigConstant` 加 2 个后缀常量、`CodeAndMsg` 加码。
3. **~~门店等级~~ 已废弃**：转让的适用门店校验并入第 9 步转让主流程（受让人 `nowStoreId` ∈ 权益卡 `store_addr_ids`），无独立步骤、不改 `store` 表。
...
- [ ] 给权益卡配置「适用门店」`store_addr_ids`（包含受让人 B 所属门店地址、刻意不含某门店地址 X 作反例）。
...
- [ ] `/api/vipTransfer/apply`（A → B，B 当前门店在适用门店内）：建 `vip_benefit_transfer`（`status=10 待付费`，`service_fee` 按档），生成服务费支付单（末位后缀 7）。前置校验：过期/不可转/黑名单/转给自己/受让人门店不在适用门店 均被对应 `ERROR_VIP_*` 拦截。
...
- **事实**：同一语义在各章码值/命名不一致，举例（均自称「从 -50 起」但分配冲突）：
  ...
  - 「适用门店校验」：原各章门店等级码（s1 `-60`、s2 `-58`、s5 `-64`、s6 `-54` 互相冲突）已随门店等级方案整体废弃，统一改用 `ERROR_VIP_STORE_NOT_APPLICABLE`（附录 A，-59，受让人门店不在适用门店范围内）。
...
- **修复**：**新增一节「附录 A：CodeAndMsg 统一码表」作为唯一事实来源**，一次性把全部 VIP 码定死（建议 -50 起连续编号，给出 枚举名→码→文案 唯一映射），各章只引用不再各自定义。务必同时统一命名（`NOT_EXIST`、`NOT_OWNER`、`STATUS_ABNORMAL`、`EXPIRED`、`NOT_TRANSFERABLE`、`TO_USER_SELF`、`STORE_NOT_APPLICABLE`、`TRANSFER_STATUS`…）。
...
- **问题**：过户把 `store_id` 改成受让人门店，但 `store_addr_id` 不动；而入场/适用门店判断常用 `store_addr_id`。改一个不改另一个,后续「适用门店」校验会出现 store_id 与 store_addr_id 指向不同门店的不一致。另外受让人当前门店的来源（`toUser.nowStoreId`，即 `store_addr_id`）若为空，适用门店校验(s5 规则16)会因受让人无所属门店直接拦截。
- **修复**：明确过户是否改 store 归属（建议**不改**——权益的适用门店由 `vip_card.store_addr_ids` 决定，归属门店保持原样更简单，适用门店校验只在校验时用受让人「当前所属门店」`nowStoreId` 即时比对）；统一 store_id/store_addr_id 的处理。

#### M-6 `audit` 复核校验调 `checkTransferable` 需要受让人当前门店，但审核期 toUser 门店可能已变/为空，且 audit 在 sys.impl 调 api.impl 的 check 方法跨模块
...
- **D.3 门店归属与适用门店校验（M-5）**：过户**不改** `vip_benefit.store_id / store_addr_id`（归属保持原样）。转让的适用门店校验只看**受让人当前所属门店**：取 `toUser.nowStoreId`（该字段实际存的就是 `store_addr_id`）判断是否 ∈ 该权益来源权益卡 `vip_benefit_card.store_addr_ids`（逗号分隔），不在范围内抛 `ERROR_VIP_STORE_NOT_APPLICABLE`；若 `toUser.nowStoreId<=0`（基本类型 `int`，未归属门店），视为不在适用门店范围、拦截（需受让人先归属到适用门店）。不再做任何门店等级比较，权益的"适用门店"始终由 `vip_benefit_card.store_addr_ids` 决定，与归属门店无关。
