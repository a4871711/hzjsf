...
健身房管理系统 **hzjsf**,前后端一体的 monorepo,三个独立子项目各自有更详细的 CLAUDE.md——**动到哪个子项目就先读它的 CLAUDE.md**,本文件只讲跨项目的大局,不重复其中细节。
...
| `uniapp/` | 移动端会员端「矢历健身」(微信小程序为主,兼 H5 / App),即用户那一半前端 | uni-app + Vue 2 + uView UI + uni-simple-router,HBuilderX 构建 | `uniapp/CLAUDE.md` |
...
- **移动端会员端**(`uniapp/`,微信小程序 / H5 / App)→ 走后端 **`api` 模块**(`/api/**`)。鉴权**不在拦截器**,而在 `BaseController` 里按 token 查 Redis(详见 `Backend/CLAUDE.md`「API 鉴权」);uniapp 侧把 token 塞进每个请求的 body/参数里(详见 `uniapp/CLAUDE.md`)。此外 `Backend/h5/` 还有若干独立 H5 页面(分享/绑定等),与 uniapp 工程是两套东西。
...
> 移动端 `uniapp/` 不在这套命令行流程里:它用 **HBuilderX** 打开目录、运行到微信小程序/H5/App,且默认直连**线上** API(`uniapp/env.js`),所以改 uniapp 通常**不需要**本地起 Backend(详见 `uniapp/CLAUDE.md`)。
...
- **uniapp 不是命令行项目**:它用 HBuilderX 编译运行(`package.json` 没有 `scripts`),且默认连**线上** API 而非本地后端——本地起好 Backend 也不会让 uniapp 自动连过来,要改 `uniapp/env.js`(详见 `uniapp/CLAUDE.md`)。
